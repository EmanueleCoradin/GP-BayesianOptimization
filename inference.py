...
from model import compute_kernel_matrix, mu_0_factory
...
@jit
def expected_improvement_acquisition_function(mu_n, sigma_n, tau):
    sigma_n_safe = jnp.where(sigma_n < 1e-6, 1e-6, sigma_n)
    z = (mu_n - tau) / sigma_n_safe

    ei = (mu_n - tau) * cdf(z) + sigma_n * pdf(z)
    ei = jnp.where(jnp.isnan(ei) | jnp.isinf(ei), 0.0, ei)
    ei = jnp.where(sigma_n < 1e-6, 0.0, ei)
    return ei
...
def numpyro_model(X, y=None):
    ...
    theta   = numpyro.sample("theta", dist.LogUniform(1e-2, 10.0).expand([d]))
    sigma   = numpyro.sample("sigma", dist.LogUniform(1e-5, 1.0))
...
def compute_posterior_from_mcmc(
    ...
    print(jnp.mean(samples["theta_0"]),jnp.mean(samples["theta"]), jnp.mean(samples["sigma"]))
